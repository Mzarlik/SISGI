...
// ACTUALIZACIÓN: Cambiamos 'tipo' por 'id_tipo_bien'
$datos_actualizar = [
...
try {
    ...
    foreach ($datos_actualizar as $columna => $valor) {
        ...
        if ($valor !== null) {
            $set_clauses[] = "`$columna` = ?";
            $params[] = $valor;
            // ACTUALIZACIÓN: Si la columna es id_tipo_bien le damos formato de Entero (i), si no, String (s)
            $types .= ($columna === 'id_tipo_bien') ? 'i' : 's'; 
        }
    }
    ...
    $params[] = $id_registro;
    $types .= 'i'; // El ID del WHERE es un entero
